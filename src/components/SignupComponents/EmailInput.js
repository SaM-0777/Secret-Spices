...
const EmailInput = ({ handleSignupAttributesChange, setIsFormValid }) => {
  ...
  return (
    <View style={emailInputStyles.container} >
      <View style={[emailInputStyles.emailInputContainer, isFocused ? { borderWidth: 2, borderColor: AppStyles.primaryColor } : {}, isTextChanged ? !isValid ? { borderWidth: 2, borderColor: AppStyles.invalidTextIndicatorColor } : {  } : { }]} >
        <TextInput onFocus={onFocus} onBlur={onBlur} onChangeText={(text) => onChangeText(text)} placeholder={'Email'} maxLength={50} cursorColor={AppStyles.secondaryColor} keyboardType={'email-address'} style={emailInputStyles.emailInput} />
      </View>
